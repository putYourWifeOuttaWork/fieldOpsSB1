...
export function useProgramCloning() {
  ...
  /**
   * Get site lineage (all sites with the same site_code across programs)
   */
  const getSiteLineage = useCallback(async (siteId: string): Promise<SiteLineageItem[] | null> => {
    setLoading(true);
    setError(null);
    
    try {
      const { data, error } = await withRetry(() => 
        supabase.rpc('get_site_lineage', {
          p_site_id: siteId
        })
      );
      ...
      if (!data.success) {
        throw new Error(data.message || 'Failed to get site lineage');
      }
      
      return data.lineage;
    } catch (err) {
      console.error('Error getting site lineage:', err);
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      setError(errorMessage);
      return null;
    } finally {
