...
const SubmissionEditPage = () => {
  ...
  // Callback for PetriForm updates
  const handlePetriFormUpdate = useCallback((data: any) => {
    setPetriObservationData(prevData => ({
      ...prevData,
      [data.formId || data.observationId]: {
        ...data,
        formId: data.formId || data.observationId
      }
    }));
    
    setPetriForms(prevForms => 
      prevForms.map(f => 
        f.id === (data.formId || data.observationId)
          ? { ...f, isValid: data.isValid, isDirty: data.isDirty || f.isDirty } 
          : f
      )
    );
    
    logger.debug(`Petri form ${data.formId || data.observationId} updated with data:`, { 
  ...
  const handleGasifierFormUpdate = useCallback((data: any) => {
    ...
    logger.debug(`Gasifier form ${data.formId || data.observationId} updated with data:`, { 
                ...
                {petriForms.map((form, index) => {
                  ...
                  return (
                    <PetriForm
                      ...
                      // Pass submission temperature and humidity as fallback values
