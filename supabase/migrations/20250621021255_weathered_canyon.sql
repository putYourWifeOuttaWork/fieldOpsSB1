/*
  # Add create_site_without_history Function
  
  1. New Feature
    - Create an RPC function that handles site creation without triggering history events
    - Function accepts all site parameters and returns the new site ID
    - Required for site creation functionality in the frontend
    
  2. Purpose
    - Provides a secure way to create sites through RPC
    - Returns appropriate response format expected by frontend code
    - Handles error cases gracefully
*/

-- Create the create_site_without_history function
...
COMMENT ON FUNCTION create_site_without_history(
  ...
  irrigation_system_type_enum, lighting_system_enum, NUMERIC, NUMERIC, NUMERIC, NUMERIC, BOOLEAN, INTEGER,
  interior_working_surface_type_enum[], microbial_risk_zone_enum, INTEGER, ventilation_strategy_enum
) IS 'Creates a new site without triggering history events, with all possible parameters';