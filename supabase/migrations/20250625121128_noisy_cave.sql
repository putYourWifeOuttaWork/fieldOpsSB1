/*
  # Add Order Index to Observations
  
  1. Changes
    - Add order_index column to petri_observations table
    - Add order_index column to gasifier_observations table
    - Update RPC functions to support ordering
    
  2. Purpose
    - Maintain consistent order of observation forms
    - Prevent reordering of forms when reloading the page
    - Preserve template ordering when creating new submissions
*/
...
-- Create an index on order_index to improve query performance
CREATE INDEX IF NOT EXISTS idx_petri_observations_order_index ON petri_observations(submission_id, order_index);
CREATE INDEX IF NOT EXISTS idx_gasifier_observations_order_index ON gasifier_observations(submission_id, order_index);

-- Update the create_submission_session RPC function to set order_index from template positions
...
COMMENT ON COLUMN petri_observations.order_index IS 'Ordering index to maintain consistent display order';
COMMENT ON COLUMN gasifier_observations.order_index IS 'Ordering index to maintain consistent display order';